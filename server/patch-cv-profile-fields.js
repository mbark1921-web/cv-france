...
function updateCvPreview(){let box=$('cvPreview');if(!box)return;let template=$('cvTemplate')?$('cvTemplate').value:'classic';box.className='cv-preview '+template;box.textContent='';let name=v('cvFullName');let role=v('cvRole');let email=v('cvEmail');let phone=v('cvPhone');let address=v('cvAddress');let profile=v('cvProfile');let experience=v('cvExperience');let skills=v('cvSkills');let education=v('cvEducation');let languages=v('cvLanguages');let title=document.createElement('h1');title.textContent=name||(lang==='ar'?'سيرتي الذاتية':'Mon CV');box.appendChild(title);if(role){let rp=document.createElement('p');rp.style.fontWeight='700';rp.textContent=role;box.appendChild(rp)}let contact=[email,phone,address].filter(Boolean).join(' · ');if(contact){let cp=document.createElement('p');cp.className='muted';cp.textContent=contact;box.appendChild(cp)}let any=role||contact||profile||experience||skills||education||languages;if(!any){let p=document.createElement('p');p.className='empty';p.textContent=T[lang].previewEmpty;box.appendChild(p);return}cvSection(box,T[lang].previewProfile,profile);cvSection(box,T[lang].previewExperience,experience);cvSection(box,T[lang].previewSkills,skills);cvSection(box,T[lang].previewEducation,education);cvSection(box,T[lang].previewLanguages,languages)}
